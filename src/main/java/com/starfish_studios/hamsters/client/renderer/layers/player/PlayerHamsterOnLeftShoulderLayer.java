...
public class PlayerHamsterOnLeftShoulderLayer<T extends Player> extends RenderLayer<T, PlayerModel<T>> {
    ...
    public PlayerHamsterOnLeftShoulderLayer(RenderLayerParent<T, PlayerModel<T>> renderLayerParent, EntityModelSet entityModelSet) {
    ...
    private void render(PoseStack poseStack, MultiBufferSource multiBufferSource, int i, T player, float f, float g, float h, float j, boolean leftShoulder) {
        ...
        EntityType.byString(compoundTag.getString("id")).filter((entityType) -> entityType == HamstersEntityType.HAMSTER).ifPresent((entityType) -> {
            poseStack.pushPose();
            poseStack.scale(leftShoulder ? 1.0F : -1.0F, 1.0F, 1.0F);
