...
public class BetterGroundPathNavigation extends GroundPathNavigation {
    ...
    public boolean canCutCorner(@NotNull BlockPathTypes pathType) {
        return pathType != BlockPathTypes.DANGER_FIRE && pathType != BlockPathTypes.DANGER_POWDER_SNOW && pathType != BlockPathTypes.DANGER_OTHER && pathType != BlockPathTypes.WALKABLE_DOOR;
    }
    ...
    protected void followThePath() {
        ...
        if (shouldAdvance || this.canCutCorner(this.path.getNextNode().type) && this.shouldTargetNextNodeInDirection(tempMobPos)) this.path.advance();
    ...
    private boolean shouldTargetNextNodeInDirection(Vec3 currentPos) {
        ...
        if (!currentPos.closerThan(nextPos, 2.0)) return false;
